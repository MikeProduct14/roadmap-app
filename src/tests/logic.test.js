...
const calcProgress = (tasks, epicId) => {
  ...
  return all.length
    ? Math.round((all.filter(t => t.status === 'done').length / all.length) * 100)
    : 0
}
...
describe('addEpic', () => {
  it('adds with auto id', () => {
    const n = addEpic(mk(), { name: 'X' })
    expect(n.epics[2].id).toBe('e3')
    expect(n.nextEpicId).toBe(4)
  })
  it('does not mutate original', () => {
    const s = mk()
    addEpic(s, {})
    expect(s.epics).toHaveLength(2)
  })
})
describe('editEpic', () => {
...
describe('deleteEpic', () => {
  it('removes epic', () => {
    expect(deleteEpic(mk(), 'e1').epics).toHaveLength(1)
  })
  it('removes all its tasks', () => {
    expect(deleteEpic(mk(), 'e1').tasks.filter(t => t.epicId === 'e1')).toHaveLength(0)
  })
  it('keeps other epic tasks', () => {
    expect(deleteEpic(mk(), 'e1').tasks.find(t => t.id === 't4')).toBeTruthy()
  })
})
describe('addTask', () => {
  it('adds with auto id', () => {
    const n = addTask(mk(), { epicId: 'e1' }, { name: 'T' })
    expect(n.tasks[4].id).toBe('t5')
    expect(n.nextTaskId).toBe(6)
  })
  it('sets parentId', () => {
    expect(addTask(mk(), { epicId: 'e1', parentId: 't1' }, { name: 'S' }).tasks[4].parentId).toBe(
      't1'
    )
  })
  it('defaults assignee', () => {
    expect(addTask(mk(), { epicId: 'e1' }, { name: 'T' }).tasks[4].assignee).toBe('Не назначен')
  })
...
describe('deleteTask', () => {
  it('removes task', () => {
    expect(deleteTask(mk(), 't4').tasks.find(t => t.id === 't4')).toBeUndefined()
  })
  ...
  it('keeps other epic tasks', () => {
    expect(deleteTask(mk(), 't1').tasks.find(t => t.id === 't4')).toBeTruthy()
  })
})
describe('collectChildIds', () => {
  it('empty for leaf', () => {
    expect(collectChildIds(mk().tasks, 't4')).toEqual([])
  })
  it('returns direct children', () => {
    const ids = collectChildIds(mk().tasks, 't1')
...
describe('reorderEpics', () => {
  it('swaps two epics', () => {
    const n = reorderEpics(mk().epics, 0, 1)
    expect(n[0].id).toBe('e2')
    expect(n[1].id).toBe('e1')
  })
  it('does not mutate original', () => {
    const e = mk().epics
    reorderEpics(e, 0, 1)
    expect(e[0].id).toBe('e1')
  })
})
describe('reorderTasks', () => {
  it('reorders within same parent', () => {
    ...
    expect(ch[0].id).toBe('t3')
    expect(ch[1].id).toBe('t2')
  })
  it('keeps other epic tasks', () => {
    expect(reorderTasks(mk().tasks, 'e1', 't1', 0, 1).find(t => t.id === 't4')).toBeTruthy()
  })
})
describe('epic progress', () => {
  it('0% when none done', () => {
    expect(calcProgress([{ id: 't1', epicId: 'e1', status: 'wip' }], 'e1')).toBe(0)
  })
  ...
  it('0% for empty epic', () => {
    expect(calcProgress([], 'e1')).toBe(0)
  })
})
describe('overdue detection', () => {
  it('past deadline non-done = overdue', () => {
    expect(isOverdue({ deadline: '2020-01-01', status: 'wip' })).toBeTruthy()
  })
  it('done task not overdue', () => {
    expect(isOverdue({ deadline: '2020-01-01', status: 'done' })).toBeFalsy()
  })
  it('future deadline not overdue', () => {
    expect(isOverdue({ deadline: '2099-01-01', status: 'wip' })).toBeFalsy()
  })
  it('no deadline not overdue', () => {
    expect(isOverdue({ deadline: '', status: 'wip' })).toBeFalsy()
  })
})
